...
public interface StoreRepository {
    ...
    @Result(property = "rating", column = "id",
            one = @One(select = "com.kshrd.warehouse_master.repository.StoreRepository.getRating"))
    @Result(property = "ratingCount", column = "id",
            one = @One(select = "com.kshrd.warehouse_master.repository.StoreRepository.getRatingCount"))
    Store createNewStore(@Param("store") StoreRequest storeRequest, Integer currentUserId);


    // tb_store does not have rating. this method is for calculating rating if does not have rating return 0
    @Select("""
    ...
    @Result(property = "ratingCount", column = "id",
            one = @One(select = "com.kshrd.warehouse_master.repository.StoreRepository.getRatingCount"))
    @Result(property = "additionalPhone", column = "id", many = @Many(select = "com.kshrd.warehouse_master.repository.StoreRepository.getAdditionalPhone"))
    ...
    @Result(property = "ratingCount", column = "id",
            one = @One(select = "com.kshrd.warehouse_master.repository.StoreRepository.getRatingCount"))
    @Result(property = "additionalPhone", column = "id", many = @Many(select = "com.kshrd.warehouse_master.repository.StoreRepository.getAdditionalPhone"))
    @Result(property = "categories", column = "id", many = @Many(select = "com.kshrd.warehouse_master.repository.StoreRepository.getCategoryListingByStoreId"))
    List<StoreRetailer> getAllStore();

    @Select("""
            SELECT phone_number FROM tb_store_phone
    ...
    @Select("""
            ...
            ORDER BY ${by} ASC;
             """)
    @Result(property = "category", column = "category_id", one = @One(select = "com.kshrd.warehouse_master.repository.StoreRepository.getCategoryByCategoryId"))
